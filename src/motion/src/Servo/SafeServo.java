...
public class SafeServo extends Servo {
    ...
    private SafeServo(Orc orc, int port, double pos0, int usec0, double pos1, int usec1) {
    ...
    public void setPulseWidth(int goal) {
        ...
        if (goal < minPWM)
            goal = minPWM;
        if (this.pwm == -99) { // special case to handle first run arm should already be at (or close) to pwm
