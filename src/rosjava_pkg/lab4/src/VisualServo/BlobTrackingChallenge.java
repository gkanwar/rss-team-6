...
public class BlobTrackingChallenge {
    ...
    public void process(Image src, Image dest, int width, int height) {
    	...
    	Set<Blob> discoveredBlobs = findBlobs(hues, hueThreshold, skipThreshold, sizeThreshold);
    ...
    public Set<Blob> findBlobs(int[][] hues, int hueThreshold, int skipThreshold, int sizeThreshold) {	
		...
		for (int x=0; x<width; x++) {
			for (int y=0; y<height; y++) {
				...
				if (!examinedPoints.contains(startPoint)) {
					Set<Point2D.Double> currentBlobPoints = findNewBlob(hues, startPoint, hueThreshold, skipThreshold);
					examinedPoints.addAll(currentBlobPoints);
					Blob processedBlob = processBlob(hues, currentBlobPoints, sizeThreshold);
    ...
    public Set<Point2D.Double> findNewBlob(int[][] hues, Point2D.Double startPoint, int hueThreshold, int skipThreshold) {
				...
				for (int xSq=-1*skipThreshold; xSq<skipThreshold+1; xSq++) {
					for (int ySq=-1*skipThreshold; ySq<skipThreshold+1; ySq++) {						
						...
						if ((xPos >= 0 && xPos <= width - 1) && 
								...
								(Math.abs(hues[yPos][xPos] - hues[yPos][xPos]) <= hueThreshold)) {
	...
	public Blob processBlob(int[][] hues, Set<Point2D.Double> blobPoints, int sizeThreshold) {
		Blob blob = null;		
