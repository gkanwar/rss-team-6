 ...
 * 
 * <p>
 * Can either be run as a stand-alone program with the provided {@link #main}
 * (recommended), or you can make an instance of it from within your own
 * program.
 * </p>
 * 
 ...
 * 
 * <p>
 * A coordinate grid is displayed representing the robot world frame. The X axis
 * is horizontal and positive to the right. The Y axis is vertical and positive
 * up. The X and Y axis lines, when in view, are displayed in a highlighted
 * color. Other grid lines are plotted at 1m intervals.
 * </p>
 * 
 ...
 * 
 * <p>
 * The most recently plotted pose will always be displayed in a highlighted
 * color. A fraction of all the older poses (controlled by
 * {@link #poseSaveInterval}) will also be plotted, in a lighter color, to
 * provide you with a representation of the history of the robot's motion.
 * </p>
 * 
 * <h2>Stand-Alone Operation</h2>
 * 
 * <p>
 * When run stand-alone, connects to the Carmen daemons directly and
 * periodically plots the robot pose. Vision images are displayed in the
 * upper-left corner as they are received. See the documentation for
 * {@link #main} for the command line arguments and other details for
 ...
 * 
 * <p>
 * When run from within your own program, this class <b>does not</b>
 ...
 * drawing methods as you desire to plot other data.
 * </p>
 * 
 * <p>
 * Note that it is generally preferable design your code to run the GUI in
 * stand-alone mode so that you can run it on a different machine than the one
 * which runs your robot's operational code.
 * </p>
 * 
 * <h3>Drawing Data Manually</h3>
 * 
 * <p>
 * When run from within your own program, you will need to write code to
 * manually draw things in the GUI.
 * </p>
 * 
 * <p>
 * Drawing example:
 ...
 * 
 * <p>
 * If {@link #maxTV} and {@link #maxRV} are both greater than zero then you can
 * drive the robot around using the keyboard:
 * <ul>
 * <li>space - stop</li>
 * <li>i - fwd</li>
 * <li>, - rev</li>
...
public class VisionGUI extends JPanel implements NodeMain {
    ...
    /**
     * <p>
     * Transform that takes points in world coordinates (meters) to points in
     ...
     * </p>
     **/
    protected java.util.List<Pose> historicalPoses = Collections
            .synchronizedList(new ArrayList<Pose>());
     ...
     * 
     * <p>
     * Zero indicates that every pose is saved. Negative indicates that no poses
     * are saved.
     * </p>
     **/
    ...
    /**
     * <p>
     * Synchronization object for {@link #poseSaveInterval} and
     * {@link #posesToGoBeforeSave}.
     * </p>
     **/
    ...
    /**
     * <p>
     * Covers {@link #VisionGUI(int)}, always uses
     * {@link #DEFAULT_POSE_SAVE_INTERVAL}.
     * </p>
     **/
    public VisionGUI() {
     ...
     * 
     * @param maxTV
     *            the max translation velocity in m/s. Less than or equal to
     ...
     * 
     * @param maxRV
     *            the max rotation velocity in rad/s. Less than or equal to zero
     *            disables driving.
     **/
     ...
     * automatically called when Carmen odometry messages are recieved.
     * </p>
     * 
     * <p>
     * If {@link #poseSaveInterval} is non-negative, then it indicates how many
     * calls to this method are skipped between a historical pose is saved.
     * </p>
     * 
     * @param x
     *            the robot center x position in world frame (m)
     * @param y
     *            the robot center y position in world frame (m)
     * @param theta
     *            the ccw orientation of the robot frame x-axis wrt the world
     *            frame x-axis (rad)
     **/
    ...
    /**
     * <p>
     * Set the line width in <code>g2d</code>.
     * </p>
     * 
     * @param g2d
     *            the Graphics2D in which to set the appearance
     * @param lineWidth
    ...
    /**
     * <p>
     * Hook to add painting in world frame over the coordinate grid but under
     * the robot poses.
     * </p>
     **/
    ...
    protected void paintGrid(Graphics2D g2d) {
        ...
        for (double x = Math.ceil(xMin / GRID_SPACING) * GRID_SPACING; x <= xMax; x += GRID_SPACING) {
            l.x1 = x;
            l.x2 = x;
            g2d.draw(l);
        }
        ...
        for (double y = Math.ceil(yMin / GRID_SPACING) * GRID_SPACING; y <= yMax; y += GRID_SPACING) {
            l.y1 = y;
            l.y2 = y;
            g2d.draw(l);
        }
    ...
    protected void paintPoseHistory(Graphics2D g2d) {
        ...
        synchronized (historicalPoses) {
            for (Iterator<Pose> it = historicalPoses.iterator(); it.hasNext();)
                it.next().paint(g2d);
        }
    }
    ...
    /**
     * <p>
     * Updates {@link #worldToView} to be centered at <code>(cx, cy)</code> with
     * the given scale.
     * </p>
     * 
     * @param cx
     *            the center x world coordinate in meters
     * @param cy
     *            the center y world coordinate in meters
     * @param scale
    ...
    /**
     * <p>
     * Updates {@link #worldToView} to be centered at <code>(cx,
     * cy)</code>.
     * </p>
     * 
     * @param cx
    ...
    protected MouseInputListener mouseInputListener = new MouseInputAdapter() {

        {
            ...
            addMouseWheelListener(new MouseWheelListener() {
                ...
                public void mouseWheelMoved(MouseWheelEvent e) {
                    ...
                    doZoom(deltaToZoom(WHEEL_TO_DELTA * e.getWheelRotation(),
                            accel), modifiers);
                }
            });
        }
        ...
        public void mousePressed(MouseEvent e) {
            ...
            Point2D.Double d = new Point2D.Double(startDragPoint.x
                    - lastClickedPoint.x, startDragPoint.y - lastClickedPoint.y);
            System.out.println("clicked point: " + startDragPoint);
            System.out.println("  rel to last: " + d);
            System.out.println("         dist: "
                    + startDragPoint.distance(lastClickedPoint));
            lastClickedPoint.setLocation(startDragPoint);
        ...
        /**
         * <p>
         * Check whether <code>modifiers</code> indicates a zoom.
         * </p>
         * 
         * <p>
        ...
        /**
         * <p>
         * Check whether <code>modifiers</code> indicates a pan.
         * </p>
         * 
         * <p>
         * Current impl just returns <code>!{@link #isZoom}</code>.
         * </p>
         * 
         * @param modifiers
         *            the extended modifiers
        ...
        /**
         * <p>
         * Check whether <code>modifiers</code> indicates an accelerated
         * navigation.
         * </p>
         * 
         * <p>
         ...
         * 
         * @param dy
         *            delta y, in viewpoint coordinates
         * @param accel
         *            whether to multiply the return value by
         *            {@link #ZOOM_ACCEL_FACTOR}
         * 
         ...
         * 
         * @param dx
         *            delta x, in viewpoint coordinates
         * @param accel
         *            whether to multiply the return value by
         *            {@link #PAN_ACCEL_FACTOR}
         * 
         ...
         * 
         * <p>
         * Zooms about {@link #zoomCenter} by default, about the view center if
         * ALT is down.
         * </p>
         * 
         * @param newScale
         ...
         * 
         * @param dx
         *            the x translation, in viewpoint coordinates
         * @param dy
         *            the y translation, in viewpoint coordinates
         **/
        ...
        protected Point2D.Double lastClickedPoint = new Point2D.Double();
    ...
    /**
     * <p>
     * Drives robot iff both {@link #maxTV} and {@link #maxRV} are greater than
     * zero.
     * </p>
     * 
     * <p>
